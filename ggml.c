...
inline static void * ggml_aligned_malloc(size_t size) {
    ...
    if (result != 0) {
        ...
        GGML_PRINT("%s: %s (attempted to allocate %6.2f MB)\n", "Info", error_desc, size/(1024.0*1024.0));
        GGML_ASSERT(false);
        return NULL;
    }
...
static struct ggml_object * ggml_new_object(struct ggml_context * ctx, enum ggml_object_type type, size_t size) {
    ...
    if (cur_end + size_needed + GGML_OBJECT_SIZE > ctx->mem_size) {
        GGML_PRINT("%s: not enough space in the context's memory pool (needed %zu, available %zu)\n",
                "Info", cur_end + size_needed, ctx->mem_size);
        assert(false);
        return NULL;
    }
        ...
        if (ctx->scratch.data != NULL) {
            ...
            if (ctx->scratch.offs + data_size > ctx->scratch.size) {
                GGML_PRINT("%s: not enough space in the scratch memory pool (needed %zu, available %zu)\n",
                        "Info", ctx->scratch.offs + data_size, ctx->scratch.size);
                assert(false);
                return NULL;
            }
...
static int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads) {
    ...
    switch (node->op) {
        ...
        default:
            {
                fprintf(stderr, "%s: op not implemented: ", "Info");
    ...
    // check the magic before making allocations
    {
        ...
        for (uint32_t i = 0; i < sizeof(magic); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                fprintf(stderr, "%s: invalid magic characters '%c%c%c%c'\n", "Info", magic[0], magic[1], magic[2], magic[3]);
                fclose(file);
                return NULL;
            }
        }
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ne % ggml_blck_size(info->type) != 0) {
                fprintf(stderr, "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64 ") is not a multiple of block size (%d)\n",
                        "Info", info->name.data, (int)info->type, ggml_type_name(info->type), ne, ggml_blck_size(info->type));
                fclose(file);
                gguf_free(ctx);
                return NULL;
            }
