...
struct whisper_params {
    int32_t n_threads = std::min(16, (((int32_t)std::thread::hardware_concurrency() - 2) > 0 ? ((int32_t)std::thread::hardware_concurrency() - 2) : 1));
...
static bool whisper_params_parse(int argc, char** argv, whisper_params& params) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-f" || arg == "--file") { params.fname_inp.emplace_back(argv[++i]); }
        else {
...
static void whisper_print_usage(int /*argc*/, char** argv, const whisper_params& params) {
    fprintf(stderr, "\n");
    fprintf(stderr, "usage: %s -f file.wav\n", argv[0]);
    fprintf(stderr, "Then wait for the progress.");
    fprintf(stderr, "\n");
}
...
static void whisper_print_progress_callback(struct whisper_context* /*ctx*/, struct whisper_state* /*state*/, int progress, void* user_data) {
    ...
    if (progress >= *progress_prev + progress_step) {
        *progress_prev += progress_step;
        fprintf(stderr, "Info: progress = %3d%%\n", progress);
    }
}
    ...
    */
    ...
    for (int f = 0; f < (int)params.fname_inp.size(); ++f) {
        ...
        if (!params.no_prints) {
            ...
            fprintf(stderr, "\n");
            fprintf(stderr, "Processing file '%s' (%d samples, %.1f sec) with %d threads, %d processors...\n", fname_inp.c_str(), int(pcmf32.size()), float(pcmf32.size()) / WHISPER_SAMPLE_RATE,
        ...
        // run the inference
        {
            ...
            if (use_grammar) {
                if (grammar_parsed.symbol_ids.find(params.grammar_rule) == grammar_parsed.symbol_ids.end()) {
                    fprintf(stderr, "%s: warning: grammar rule '%s' not found - skipping grammar sampling\n", "Info", params.grammar_rule.c_str());
                }
                else {
