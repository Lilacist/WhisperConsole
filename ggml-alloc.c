...
void ggml_tallocr_alloc(struct ggml_tallocr * talloc, struct ggml_tensor * tensor) {
    ...
    if (talloc->offset + size > ggml_backend_buffer_get_size(talloc->buffer)) {
        fprintf(stderr, "%s: not enough space in the buffer to allocate %s (needed %zu, available %zu)\n",
                "Info", tensor->name, size, ggml_backend_buffer_get_size(talloc->buffer) - talloc->offset);
...
static size_t ggml_dyn_tallocr_alloc(struct ggml_dyn_tallocr * alloc, size_t size, const struct ggml_tensor * tensor) {
    ...
    AT_PRINTF("%s: allocating %s (%zu bytes) - ", "Info", tensor->name, size);
...
static void ggml_dyn_tallocr_free_tensor(struct ggml_dyn_tallocr * alloc, size_t offset, size_t size, const struct ggml_tensor * tensor) {
    ...
    AT_PRINTF("%s: freeing %s at %zu (%zu bytes) - n_free_blocks = %d\n", "Info", tensor->name, offset, size, alloc->n_free_blocks);
...
#ifndef NDEBUG
            //fprintf(stderr, "%s: reallocating %s buffer from size %.02f MiB to %.02f MiB\n", "Info", ggml_backend_buft_name(galloc->bufts[i]), cur_size / 1024.0 / 1024.0, new_size / 1024.0 / 1024.0);
#endif

            ggml_backend_buffer_free(galloc->buffers[i]);
            galloc->buffers[i] = ggml_backend_buft_alloc_buffer(galloc->bufts[i], new_size);
            if (galloc->buffers[i] == NULL) {
                fprintf(stderr, "%s: failed to allocate %s buffer of size %zu\n", "Info", ggml_backend_buft_name(galloc->bufts[i]), new_size);
                return false;
            }
        }
    }
...
static bool alloc_tensor_range(struct ggml_context * ctx,
    ...
    if (buffer == NULL) {
#ifndef NDEBUG
        fprintf(stderr, "%s: failed to allocate %s buffer of size %zu\n", "Info", ggml_backend_buft_name(buft), size);
#endif
